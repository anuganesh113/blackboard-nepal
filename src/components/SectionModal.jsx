...
const SectionModal = ({ isOpen, onClose, section, onSave, mode = 'edit' }) => {
  ...
  const validate = () => {
    ...
    // Validate that section name is a single character or short name
    if (formData.name.trim().length > 10) {
      newErrors.name = 'Section name should be short (max 10 characters)';
    }
    return newErrors;
  };
  ...
  const handleSubmit = (e) => {
    ...
    // Add current date for createdOn/modifiedOn
    ...
    // If creating new section, add createdOn
    if (mode === 'create') {
      sectionData.createdOn = currentDate;
    }
          ...
          <div className={`flex items-center justify-between p-6 border-b bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-t-2xl ${
            ...
            <div className="flex items-center gap-3">
              ...
              <h2 className={`text-2xl font-bold ${
                ...
                {mode === 'edit' ? 'Update Section' : 'Add New Section'}
              </h2>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              <label 
                ...
                className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2"
              >
                Active Status <span className="text-red-500 ml-1">*</span>
              </label>
              <select
                id="isActive"
                name="isActive"
